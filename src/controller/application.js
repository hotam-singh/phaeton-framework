...
class Application {
	...
	constructor(genesisBlock, config = {}) {
		const errors = phaetonValidator.validate(genesisBlockSchema, genesisBlock);
		if (errors.length) {
			throw errors;
		}
		...
		if (!_.has(appConfig, 'app.label')) {
			_.set(
				appConfig,
				'app.label',
				`phaeton-${genesisBlock.payloadHash.slice(0, 7)}`,
			);
		}
		...
		if (!_.has(appConfig, 'components.logger.logFileName')) {
			_.set(
				...
				`${process.cwd()}/logs/${appConfig.app.label}/phaeton.log`,
			);
		}
	 ...
	 */
	async run() {
		this.logger.info(`Booting the application with Phaeton Framework(${version})`);
	...
	_compileAndValidateConfigurations() {
		...
		const childProcessModules = process.env.PHAETON_CHILD_PROCESS_MODULES
			? process.env.PHAETON_CHILD_PROCESS_MODULES.split(',')
			: [];
